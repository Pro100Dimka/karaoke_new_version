...
| demucs, openai-whisper, torchcrepe (extra `ai`) | локальный AI-провайдер `local-torch`: разделение голоса и музыки (HTDemucs), распознавание речи и тайминги слов (Whisper base, uk/ru/en), высота тона (CREPE) | собственные модели писать нельзя (п. 33); все три работают на torch — второй ML runtime не добавляется (п. 24); веса Demucs и Whisper скачиваются штатным механизмом моделей приложения с проверкой размера и sha256 (`backend/ai/catalog.py`), веса CREPE входят в пакет | только отдельный процесс `backend/ai_worker` (не API-процесс) |
