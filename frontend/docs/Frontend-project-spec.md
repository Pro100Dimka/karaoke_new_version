...
# 143. Karaoke display modes

Пользователь может выбирать отображение:
...
Недоступные режимы автоматически исключаются, если song project не содержит необходимых данных.

Последний выбранный display mode сохраняется как пользовательская preference, но для несовместимой песни используется ближайший доступный fallback.
