...
export const RoomTransferStatus = ({ room, onCancel, onRetry }: {
  room: RoomStateDto; onCancel(): void; onRetry(): void;
}) => {
  const t = useText();
  ...
  const key = room.transferError ? "retry" : room.transferId ? "cancel" : undefined;
  ...
  return <div className="transfer">
    ...
    {room.transferTotalBytes !== undefined && <Typography as="span" variant="caption" tone="muted">
      {formatBytes(room.transferBytes ?? 0)} / {formatBytes(room.transferTotalBytes)}
    </Typography>}
    {action && <Button size="sm" variant="outlined" startIcon={<action.icon size={14} />} onClick={action.run}>
      {t(action.label)}
    </Button>}
  </div>;
};
