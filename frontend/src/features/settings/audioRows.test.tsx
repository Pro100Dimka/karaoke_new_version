...
describe("audio settings rows", () => {
  it("shows only sample rates and periods reported by the selected device", () => {
    const rows = audioRows(
      ...
      { backend: "WASAPI Shared", sampleRate: 0, periodFrames: 0, bufferFrames: 0, inputDeviceId: "", outputDeviceId: "" },
    ...
    expect(rows.some(row => "tag" in row && row.tag === "bufferFrames")).toBe(false);
  });
  ...
  it("shows a buffer field for exclusive and ASIO backends", () => {
    for (const backend of ["WASAPI Exclusive", "ASIO"] as const) {
      const rows = audioRows(
        ((key: string) => key) as never,
        { backend, sampleRate: 48000, periodFrames: 441, bufferFrames: 256, inputDeviceId: "", outputDeviceId: "" },
      ...
      const row = rows.find(candidate => "tag" in candidate && candidate.tag === "bufferFrames") as { label?: string };
      expect(row.label).toBe("audioBuffer");
      expect(rows.some(candidate => "tag" in candidate && candidate.tag === "periodFrames")).toBe(false);
    }
  });
});
