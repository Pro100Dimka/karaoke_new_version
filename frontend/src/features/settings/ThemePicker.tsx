...
export const ThemePicker = ({ value, disabled, onChange }: ThemePickerProps) => {
  const t = useText();
  return (
    <div className="themeGrid" role="group" aria-label={t("theme")}>
      {themeOptions.map(option => (
        <Button key={option.value} type="button" unstyled className="themeOption" disabled={disabled} aria-pressed={value === option.value} onClick={() => onChange(option.value)}>
